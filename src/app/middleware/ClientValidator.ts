    ...
    return async (request: Request, response: Response, next: NextFunction) => {
      try {
        ...
        arraySector.forEach((string, index) => {
          request.body[string].cpf = request.body[string].cpf.replace(
            /\D/g,
            ""
          );
          request.body[string].rg = request.body[string].rg.replace(/\D/g, "");
          console.log(
            `esse é o setor ${string}, com o seu cpf todo sanitizado após a sua limpeza ${request.body[string].cpf}`
          );
          console.log(
            `esse ainda é o setor ${string}, com o seu rg todo sanitizado após a sua limpeza ${request.body[string].rg}`
          );
        });
