...
export class ClientRepository {
  ...
  static async getAllByStatus(limit: number, page: number, status: boolean) {
    try {
  ...
  static async showCLients(limit: number, page: number) {
    try {
      const allclients: allClientes[] =
        await InstanciaPrisma.GetConnection().client.findMany({
          include: {
  ...
  static async filterClientErpHomologation({
    branch_activity,
    phone,
    name,
    email,
  ...
  static async filterClient({
    ...
    try {
      ...
      console.log(
        `ramo de atuação : ${branch_activity.contains}, nome_FANTASIA: ${fantasy_name.contains}, email : ${email.contains}, name: ${name.contains}, phone : ${phone.contains}`
      );
