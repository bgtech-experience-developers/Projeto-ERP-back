...
class FuncionariosRepository {
  async criar(nome, email, cpf, telefone, celular) {
    try {
      const funcionario = await instanciaPrisma.fucionarios.findUnique({
        where: { cpf },
      });
      if (!funcionario) {
        const cadastrarFuncionario = await instanciaPrisma.fucionarios.create({
          data: { nome, email, cpf, celular, telefone },
        });
  ...
  async atualizarFuncionario(nome, email, cpf, celular, telefone) {
    ...
    try {
      if (funcionario) {
        const funcionarioNovo = await instanciaPrisma.fucionarios.update({
          where: { cpf },
          data: { nome, email, cpf, celular, telefone },
        });
        return {
